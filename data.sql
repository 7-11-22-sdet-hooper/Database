...
(7,"inheritance","One of the four pillars of OOP, this solves the problem of redundant code by allowing more specific versions of a class to use an existing implementation of fields/m0ethods"),
...
(7,"encapsulation","One of the four pillars of OOP, this focuses on organizing each class's fields and methods so that each class only needs to worry about its own data and actions necessary to exist"),
(1,"this","Java keyword which disambiguates an object's fields when method parameters have the same name"),
(1,"void","Java keyword indicating a method returns nothin'g"),
(1,"abstract","Java keyword for a (i) class that can't be instantiated or (ii) a method whose implementation isn't defined in said class"),
(2,"JOIN","SQL keyword used to re-combine tables, usually with one table's foreign key linking together with another table's primary key"),
...
(3,"Waterfall Method","What SDLC is heavily structured and resistant to change?"),
(1,"final","A modifier used for classes, variables, and methods, which makes them non-changeable (impossible to inherit or override)"),
...
(3,	"What do we use to describe the complexity or performance of an algorithm? ",	"Verbosity",	"Little S notation",	"Big O notation",	"Performance Scalability Range",	"C"),
(1,	"Which Java data structure stores values in pairs and can be searched by a key?", "HashSet",	"Binary Tree",	"Dictionary",	"HashMap",	"D"),
...
(4,	"Which type of testing is done to verify how well the application works under stress (or above the maximum supported load)?",	"Functional Tests",	"Stress", 	"Integration/Service Tests",	"Unit Tests",	"B"),
...
(4,	"Tests that check to see if all parts of the program are accessible and if they require any specific augmentations audience",	"Stress", 	"Performance",	"Functional Tests",	"Accessibility",	"D");